...
class Runner(object):
    def load_data(self):
        ...
        self.p.embed_dim = (
            self.p.k_w * self.p.k_h if self.p.embed_dim is None else self.p.embed_dim
        )
        ...
        self.data_iter = {
            ...
            "valid_head": get_data_loader(
                TestDataset, "valid_head", self.p.test_batch_size
            ),
            "valid_tail": get_data_loader(
                TestDataset, "valid_tail", self.p.test_batch_size
            ),
            "test_head": get_data_loader(
                TestDataset, "test_head", self.p.test_batch_size
            ),
            "test_tail": get_data_loader(
                TestDataset, "test_tail", self.p.test_batch_size
            ),
        }
    ...
    def fit(self):
        ...
        """
        try:
            self.best_val_mrr, self.best_val, self.best_epoch, val_mrr = 0.0, {}, 0, 0.0
            ...
            val_results = {}
            val_results["mrr"] = 0
            for epoch in tqdm(range(self.p.max_epochs)):
            ...
            self.save_model(save_path + f"_final_{self.p.max_epochs}_epochs")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-name", default="testrun", help="Set run name for saving/restoring models"
    )
    parser.add_argument(
        "-data",
        dest="dataset",
    ...
    parser.add_argument(
        "-epoch", dest="max_epochs", type=int, default=500, help="Number of epochs"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "-seed", dest="seed", default=41504, type=int, help="Seed for randomization"
    )
    ...
    parser.add_argument(
        "-hid_drop", dest="hid_drop", default=0.3, type=float, help="Dropout after GCN"
    )
    ...
    parser.add_argument(
        "-ker_sz", dest="ker_sz", default=7, type=int, help="ConvE: Kernel size to use"
    )

    parser.add_argument(
        "-logdir", dest="log_dir", default="./log/", help="Log directory"
    )
    parser.add_argument(
        "-config", dest="config_dir", default="./config/", help="Config directory"
    )
    ...
    parser.add_argument(
        "-strategy", type=str, default="one_to_n", help="Training strategy to use"
    )
    parser.add_argument(
        "-form", type=str, default="plain", help="The reshaping form to use"
    )
    parser.add_argument(
        "-ik_w", dest="ik_w", default=10, type=int, help="Width of the reshaped matrix"
    )
    parser.add_argument(
        "-ik_h", dest="ik_h", default=20, type=int, help="Height of the reshaped matrix"
    )
    parser.add_argument(
        "-inum_filt",
        dest="inum_filt",
    ...
    parser.add_argument(
        "-iker_sz", dest="iker_sz", default=9, type=int, help="Kernel size to use"
    )
    parser.add_argument(
        "-iperm",
        dest="iperm",
        default=1,
        type=int,
        help="Number of Feature rearrangement to use",
    )
    parser.add_argument(
        "-iinp_drop",
        dest="iinp_drop",
